...
<template>
  <main class="max-w-3xl mx-auto px-4 py-8">
    ...
    <div v-if="loading" class="mt-4 text-slate-600">読み込み中…</div>
    ...
    <div v-else-if="project" class="mt-6 space-y-4">
      ...
      <img v-if="project.image_url" :src="project.image_url" :alt="project.title"
           class="rounded-xl border w-full max-h-[360px] object-cover" />
      ...
      <div class="flex gap-3">
        <a v-if="project.app_url" :href="project.app_url" target="_blank" class="rounded-lg bg-indigo-600 text-white px-3 py-1.5 text-sm">Open App</a>
      ...
      <section v-if="project.story_md">
        ...
        <pre class="mt-2 whitespace-pre-wrap text-sm bg-white/70 border rounded-xl p-3">{{ project.story_md }}</pre>
      </section>
    </div>
  </main>
</template>
